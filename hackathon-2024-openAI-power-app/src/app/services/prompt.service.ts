...
export class PromptService {
  ...
  private getEventPrompt() {
    return "An event represents a real-world event at a venue.  " + 
      ...
      "Organization is a string with two characters.  If the user doesn't mention an Org or Organization or Org Code with two characters, default the value to D2.  " +
      ...
      "Status is a two character string that will be set to AI.";
  }

  private getFunctionPrompt() {
    return "Events can have multiple functions.  Functions are similar to events, but like they are like smaller events within the event.  " +
      "They will have the keys ID, Description, EventID, StartDate, EndDate, StatusCode." +
      "EventID will be the ID from the Event.  Description is a string that describes the function.  StartDate is a datetime that represents the date the function starts.  " +
